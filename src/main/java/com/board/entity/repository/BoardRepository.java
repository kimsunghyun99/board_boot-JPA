...
public interface BoardRepository extends JpaRepository<BoardEntity, Long> {
	...
	/*게시물 목록 보기
	  select seq, seqno, writer, title, regdate, hitno 
		from ( select row_number() over(order by seqno desc) as seq, seqno, writer, title,to_char(regdate,'YYYY-MM-DD HH24:MI:SS') as regdate, hitno from tbl_board 
		where writer like '%'||#{keyword}||'%' or title like '%'||#{keyword}||'%' or content like '%'||#{keyword}||'%') 
		where seq between #{startPoint} and #{endPoint}
	 */
	
	
	// 게시물 목록 보기   select = findBy
